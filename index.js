...
const wallpapperDirList = fs
  .readdirSync(wallpapperDir)
  .filter((dir) => {
...
async function outputFile(dirList) {
  fs.rmSync("imgs", { force: true, recursive: true });
  fs.mkdirSync("imgs");
  for (const dir of dirList) {
    ...
    try {
      ...
      for (file of files) {
        console.log(file);
        fs.renameSync(
          path.resolve(materials, file),
          path.resolve(".", "imgs", uid++ + file)
        );
